...
import { Stack, Heading, Button, Badge } from '@vibe-architecture/react';
interface HeaderProps { cartCount: number; user: any; onNavigate: (page: string) => void; onLogout: () => void; }
export const Header: React.FC<HeaderProps> = ({ cartCount, user, onNavigate, onLogout }) => (
  <Stack direction="row" justify="space-between" align="center" style={{ marginBottom: 20, paddingBottom: 16 }}>
    <Heading level={1}>Shopping Mall <Badge tone="subtle" size="sm">Atomic Design</Badge></Heading>
    <Stack direction="row" gap="var(--va-space-12)" align="center">
      ...
      {user ? (
        <><Text>{user.name}</Text><Button variant="ghost" onClick={onLogout}>로그아웃</Button></>
      ) : (
        <Button variant="solid" onClick={() => onNavigate('login')}>로그인</Button>
      )}
    </Stack>
  </Stack>
);
