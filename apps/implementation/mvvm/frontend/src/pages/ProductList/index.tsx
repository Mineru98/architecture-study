...
export default function ProductList() {
  const { products, category, setCategory, loading } = useProductListPresenter();
  ...
  return (
    <div>
      ...
      <Stack direction="row" gap="var(--va-space-8)" style={{ padding: 20, flexWrap: 'wrap' }}>
        ...
        {CATEGORIES.map((c) => <Button key={c} variant={category === c ? 'solid' : 'outline'} onClick={() => setCategory(c)}>{c}</Button>)}
      </Stack>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(240px, 1fr))', gap: 16, padding: 20 }}>
        {products.map((p) => (
          <Card key={p.id} style={{ cursor: 'pointer' }} onClick={() => navigate(`/products/${p.id}`)}>
            ...
            <Text style={{ fontSize: 18, fontWeight: 700, marginTop: 8 }}>{p.priceLabel}</Text>
            <Text variant="caption">{p.stockLabel}</Text>
          </Card>
        ))}
      </div>
    </div>
