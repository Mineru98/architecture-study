...
export default function Cart() {
  const { items, totalLabel, canCheckout, removeItem, updateQuantity, onCheckout } = useCartPresenter();
      ...
      {items.length === 0 ? <Text>장바구니가 비어있습니다.</Text> : (
        <>
          {items.map((item) => (
            <Stack direction="row" justify="space-between" align="center" key={item.productId} style={{ padding: '12px 0', borderBottom: '1px solid #eee' }}>
              <div>
                ...
                <Text variant="muted">{item.subtotalLabel}</Text>
              </div>
          ...
          <div style={{ fontSize: 20, fontWeight: 700, marginTop: 16 }}>
            <Text style={{ fontWeight: 'bold' }}>합계: {totalLabel}</Text>
          </div>
          <Button variant="solid" onClick={onCheckout} disabled={!canCheckout} style={{ width: '100%', marginTop: 20, padding: 14, fontSize: 16 }}>
